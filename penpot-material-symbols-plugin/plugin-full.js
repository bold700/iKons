...
class MaterialSymbolsPlugin {
    ...
    renderIcons() {
        ...
        // Show total count
        const totalCount = MATERIAL_SYMBOLS.length;
        console.log(`Loaded ${totalCount} Material Symbols icons`);
    }
    ...
    filterIcons(searchTerm) {
        ...
        if (visibleCount === 0 && searchTerm.trim() !== '') {
            if (!noResultsMsg) {
                ...
                noResultsMsg.textContent = `No icons found for "${searchTerm}". Try a different search term.`;
        ...
        console.log(`Search "${searchTerm}": ${visibleCount} icons found`);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    const plugin = new MaterialSymbolsPlugin();
    
    // Show total icon count in console
    console.log(`🎨 Material Symbols Plugin loaded with ${MATERIAL_SYMBOLS.length} icons`);
});
